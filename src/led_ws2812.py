...
# Define LED strip parameters
    ...
    # Create an instance of the LED strip
        # strip = libs.Adafruit_NeoPixel(LED_COUNT,LED_PIN,LED_FREQ_HZ,LED_DMA,LED_INVERT,LED_BRIGHTNESS,LED_CHANNEL)
        strip = libs.PixelStrip(LED_COUNT,LED_PIN,LED_FREQ_HZ,LED_DMA,LED_INVERT,LED_BRIGHTNESS,LED_CHANNEL,strip_type=None, gamma=None)
        ...
        self.effect = None
        self.effect_lock = libs.threading.Lock()        
        self.led_thread = libs.threading.Thread(target=self.run_led_thread)
    ...
    def wheel(self,pos):
        ...
        if pos < 85:
            return libs.Color(pos * 3, 255 - pos * 3, 0)
        elif pos < 170:
            pos -= 85
            return libs.Color(255 - pos * 3, 0, pos * 3)
        else:
            pos -= 170
            return libs.Color(0, pos * 3, 255 - pos * 3)
            
    def colorWipe(self,Color, wait_ms=5):
        """Wipe libs.Color across display a pixel at a time."""
        for i in range(self.strip.numPixels()):
            self.strip.setPixelColor(i, Color)
            self.strip.show()
            libs.time.sleep(wait_ms / 300) 
            
    def effect_1(self,Color=libs.Color(libs.random.randint(0,255),libs.random.randint(0,255),libs.random.randint(0,255)), wait_ms=100):
        """Wipe libs.Color across display a pixel at a time."""
        for i in range(self.strip.numPixels()):
            self.strip.setPixelColor(libs.random.randint(0,self.strip.numPixels()-i), libs.Color)
            self.strip.show()
            libs.time.sleep(wait_ms / 300)         
    ...
    def effect_3(self,wait_ms=20):
        colors = []
        for i in range(self.strip.numPixels()):
            r, g, b = libs.colorsys.hsv_to_rgb(i / float(self.strip.numPixels()), 1, 1)
            colors.append(libs.Color(int(255 * r), int(255 * g), int(255 * b)))      
    ...
    def effect_4(self):
        for i in range(16):
            pixel_index = libs.random.randint(0, self.strip.numPixels() - 1)
            pixel_index = libs.random.randint(0, self.strip.numPixels() - 1)
            self.strip.show()
            libs.time.sleep(0.1)
            self.strip.setPixelColor(pixel_index, libs.Color(0, 0, 0))  # Turn off the pixel
            self.strip.show()
            libs.time.sleep(0.1)
    ...
    def _off(self): 
        self.colorWipe(libs.Color(0,0,0))
    def _mute(self):
        self.colorWipe(libs.Color(self.hex_to_rgb(self.muted_color)[0], self.hex_to_rgb(self.muted_color)[1], self.hex_to_rgb(self.muted_color)[2]))
    def _wakeup(self):
                    
                     
        self.colorWipe(libs.Color(self.hex_to_rgb(self.wakeup_color)[0], self.hex_to_rgb(self.wakeup_color)[1], self.hex_to_rgb(self.wakeup_color)[2]))
    def _listen(self):
    ...
    def volume_change(self, volume1, volume2):
        ...
        for j in range(1):
            for q in range(1, NUMLED2 + 1):
                for i in range(q):
                    self.strip.setPixelColor(i, libs.Color(libs.random.randint(0,255),libs.random.randint(0,255),libs.random.randint(0,255)))
                self.strip.show()
                libs.time.sleep(0.15)
                for i in range(q):
            ...
            with self.effect_lock:
                if self.effect is not None:
                    if self.effect == "OFF":
                        ...
                        self.effect_4()                                          
                    self.strip.show()
            libs.time.sleep(0.1)  # Đợi một chút để tránh quá tải CPU
